...
from app.schemes.purchase_scheme import PurchaseRequest
...
@router.post("/shop/daily_shop/item/{item_id}/buy", status_code=status.HTTP_202_ACCEPTED)
async def buy_item_of_the_daily_shop(item_id: str, purchase_request: PurchaseRequest, user: User = Depends(get_current_user)):
    return await ShopService().buy_item_of_the_daily_shop(item_id, purchase_request.purchase_id, user)