...
class FollowService:
    ...
    async def follow_user(self, followed_id: str, user: User):
        ...
        search = await FollowRepository.search_existing_follow("follower_id", objectid_user_id, "followed_id", objectid_followed_id)
    ...
    async def unfollow_user(self, unfollow_id: str, user: User):
        ...
        objectid_user_id = id_converter(user.id)
        search = await FollowRepository.search_existing_follow("follower_id", objectid_user_id, "followed_id", objectid_unfollow_id)
        if search is None:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="You aren't following this user")
        
        unfollow = await FollowRepository.delete_follow("follower_id", objectid_user_id, "followed_id", objectid_unfollow_id)
        if unfollow == 0:
    ...
    async def get_followers(self, user_id: str):
        ...
        if search is None:
            return {"followers": []}
        followers_id_list = [id_converter(follow.follower_id) for follow in search]
    ...
    async def get_following(self, user_id: str):
        ...
        if search is None:
            return {"following": []}
        following_id_list = [id_converter(follow.followed_id) for follow in search]
        search_usernames = await UserRepository.search_username_list_by_id(following_id_list)
        return {"following": search_usernames}

    async def get_followers_numbers(self, user_id: ObjectId):
    ...
    async def get_following_numbers(self, user_id: ObjectId):
